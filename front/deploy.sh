...
if [[ "$confirm" =~ ^[Yy]$ ]]; then
  if gcloud beta builds triggers describe chords-explorer \
      ...
      LATEST_SECRET_CONTENT=$(gcloud secrets versions access latest --secret="chords-explorer")
      ...
      if [ "$LATEST_SECRET_CONTENT" != "$CURRENT_ENV_CONTENT" ]; then
        ...
        for VERSION_ID in $(gcloud secrets versions list chords-explorer --filter="state != disabled" --format="value(name)" --sort-by="~createTime"); do
          gcloud secrets versions disable $VERSION_ID --secret="chords-explorer"
        done
        gcloud secrets versions add chords-explorer --data-file=$env_file
      else
        printf "$env_file content matches last secret version. Skipping env update...\n"
      fi
      printf "INFO: Trigger found, deploying...\n"
      gcloud beta builds triggers run chords-explorer \
        --project="$GCLOUD_PROJECT_ID" \
        --region="europe-west9" \
    ...
    gcloud secrets create chords-explorer \
    ...
    gcloud artifacts repositories create "chords-explorer-repository" \
      --location=europe \
      --repository-format=docker \
      --description="Images for building chords-explorer Cloud Run service"
    ...
    gcloud beta builds triggers create github \
        ...
        --project="$GCLOUD_PROJECT_ID" \
        --repo-owner="$(echo "hourlier96/chords-explorer" | cut -d'/' -f1)" \
        --repo-name="$(echo "hourlier96/chords-explorer" | cut -d'/' -f2)" \
