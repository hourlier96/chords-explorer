    ...
    <v-row>
      ...
      <v-col cols="5">
        ...
        <PianoKeyboard
          ...
          :disabled="lastChordPlayedFromAnalyze"
          @add-note="
            (note) => recalculateChord(note, currentlyEditingChord, false)
          "
          @remove-note="
            (note) => recalculateChord(note, currentlyEditingChord, true)
          "
        />
...
watch(
  ...
  (newProgressionFromStore) => {
    localProgression.value = JSON.parse(
      JSON.stringify(newProgressionFromStore),
    );
  },
  { deep: true },
);
...
async function analyzeProgression() {
  ...
  try {
    ...
    const progressionSnapshot = JSON.parse(
      JSON.stringify(localProgression.value),
    );
...
const currentlyEditingChord = computed({
  ...
  set(newValue) {
    ...
    const index = localProgression.value.findIndex(
      (c) => c.id === editingChordId.value,
    );
    if (index !== -1) {
...
function recalculateChord(noteClicked, currentChord, remove = false) {
  ...
  if (remove) {
    allNotesWithOctave = selectedChordNotes.value.filter(
      (n) => n !== noteClicked,
    );
  } else {
    allNotesWithOctave = [
      ...new Set([...selectedChordNotes.value, noteClicked]),
    ];
  }
  selectedChordNotes.value = allNotesWithOctave.sort(
  ...
  const allUniqueNoteNames = [
    ...new Set(allNotesWithOctave.map((note) => note.slice(0, -1))),
  ];
  ...
  for (const potentialRootName of allUniqueNoteNames) {
    const normalizedRoot =
      ENHARMONIC_EQUIVALENTS[potentialRootName] || potentialRootName;
    ...
    const sorted_formulas = Object.entries(CHORD_FORMULAS_NORMALIZED).sort(
      ([, formulaA], [, formulaB]) =>
        (formulaB as number[]).length - (formulaA as number[]).length,
    );
    for (const [quality, formula] of sorted_formulas) {
      ...
      if (isMatch) {
        ...
        const normalizedLowestNote =
          ENHARMONIC_EQUIVALENTS[lowestNoteName] || lowestNoteName;
